\section{Introduction}
    ...
    \subsection{The Automorphic Side}
        ...
        It is now an essential technicality that we work with $\ell$-adic local systems of rank $1$ (for which we shall write $\Shv_{\underline{\bar{\Q}_{\ell}}}^{\ad, 1}(-)$) instead of simply with a generic sheaf theory $\Shv(-)$ as we have until this moment. Via the Hecke operators, one can define the Hecke eigensheaves that we eluded to earlier: as the name suggests, these are nothing but sheaves $\E \in \Shv_{\underline{\bar{\Q}_{\ell}}}^{\ad, 1}(\Bun_{\GL_1}(X))$ that are \say{eigenvectors} of the \textit{global} Hecke operators $\scrH_X$, i.e. for each such $\E$, there exists $\calL \in \Shv_{\underline{\bar{\Q}_{\ell}}}^{\ad, 1}(X)$ such that\footnote{Here, $\boxtimes$ denotes the Deligne tensor product in the $1$-category of finite abelian categories and right-exact functors; in this particular case, we shall be content with $\calL \boxtimes \E \cong \pr_1^*\calL \tensor \pr_2^*\E$.}:
            $$\scrH_X(\E) \cong \calL \boxtimes \E$$
        Now, because $\calL$ is an $\ell$-adic local systems of rank $1$ on $X$, its stalk $\calL_x$ at any point $x \in X$ is nothing but $\bar{\Q}_{\ell}$. Consequently, the corresponding local Hecke operators $\scrH_x$ admit $(\bar{\Q}_{\ell})_x \in \Shv(X)$ - the skyscraper sheaf with value $\bar{\Q}_{\ell}$ and supported at $x \in X$ - as an \say{eigenvalue}:
            $$\scrH_x(\E) \cong (\bar{\Q}_{\ell})_x \boxtimes \E$$
        (and thus one may think of $\calL$ as a family of eigenvalues of $\E$ parametrised by points $x \in X$). It is easy to see that Hecke eigensheaves form a full symmetric monoidal subcategory of $\Shv_{\underline{\bar{\Q}_{\ell}}}^{\ad, 1}(\Bun_{\GL_1}(X))$, which we shall denote by $\Eig^{\ad, 1}_{\underline{\bar{\Q}_{\ell}}}(\Bun_{\GL_1}(X))$. 
