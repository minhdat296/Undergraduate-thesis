\section{The Automorphic Side}
    ...
    \subsection{Grothendieck's Sheaf-Function Correspondence and Artin Reciprocity}
        ...
        \subsubsection{Traces of Frobenii and Grothendieck's Sheaf-Function Correspondence via character sheaves}
            ...
            \begin{lemma}[Grothendieck's Sheaf-Function Correspondence for connected groups] \label{lemma: sheaf_function_correspondence_for_connected_groups}
            ...
            \end{lemma}
            It turns out that Hecke eigensheaves on $\Bun_{\GL_1}(X)$, which is rather fortunate for us, as this directly implies that Hecke eigensheaves correspond to certain continuous $\ell$-adic characters of $\Bun_{\GL_1}(X)(\F_q)$ via lemma \ref{lemma: sheaf_function_correspondence_for_connected_groups}. Nevertheless, there remains a technical difficulty that we need to overcome, that being the fact that $\Bun_{\GL_1}(X)$ is not connected (cf. remark \ref{remark: geometry_of_the_picard_stack}).
            \begin{remark}[Identity components of smooth group schemes] \label{remark: identity_components_of_smooth_group_schemes}
            ...
            \begin{theorem}[Grothendieck's Sheaf-Function Correspondence for smooth groups] \label{theorem: sheaf_function_correspondence_for_smooth_groups}
                \cite[Theorem 3.6]{cunningham_roe_function_sheaf_dictionary_quasi_characters_p_adic_tori} If $G$ is a smooth commutative group scheme over $\Spec \F_q$ then $\trace(\Frob_G^* \mid -)$ as in lemma \ref{lemma: sheaf_function_correspondence_for_connected_groups} will be a surjective group homomorphism.
            \end{theorem}
                \begin{proof}
                    The short exact sequence from remark \ref{remark: identity_components_of_smooth_group_schemes} can be shown (see \cite[Proposition 3.3, Lemma 3.4, and Proposition 3.5]{cunningham_roe_function_sheaf_dictionary_quasi_characters_p_adic_tori}) to induce the following short exact sequence of abelian groups:
                        $$
                            \begin{tikzcd}
                ...
                \end{proof}
            \begin{remark}
                It can also be shown that $\ker \trace(\Frob_G^* \mid -) \cong H^2(\pi_0(G_{\bar{\F}_q}), \bar{\Q}_{\ell}^{\x})^{\rmW_{\F_q}}$ (here, $\rmW_{\F_q}$ denotes the subgroup of $\Gal(\bar{\F}_q/\F_q)$ that is generated by $\Frob_{\F_q}$). For details, see \cite[Proposition 2.7]{cunningham_roe_function_sheaf_dictionary_quasi_characters_p_adic_tori}
            \end{remark}
        ...
        \subsubsection{Artin Reciprocity for global function fields over finite fields}
            ...
            \end{convention}
            \begin{proposition}[Hecke eigensheaves are character sheaves] \label{prop: hecke_eigensheaves_are_character_sheaves}
                If $\cev{H}_X: \Bun_{\GL_1}^{(1)}(X) \x \Bun_{\GL_1}(X) \to \Bun_{\GL_1}(X)$ is the group operation on $\Bun_{\GL_1}(X)$, given by $\cev{H}_X(\calL', \calL) \cong \calL' \tensor_{\calO_X} \calL$, then any Hecke eigensheaf $\E \in \Eig\Shv_{\underline{\bar{\Q}_{\ell}}}^1(\Bun_{\GL_1}(X))$ satisfies the character sheaf property with respect to $\cev{H}_X$.
            \end{proposition}
                \begin{proof}
                    We have already shown that Hecke eigensheaves are Weil sheaves in lemma \ref{lemma: hecke_eigensheaves_are_weil_sheaves}, so the only thing to prove is that $\cev{H}_X^*\E_{\calL} \cong \E_{\calL} \boxtimes \E_{\calL}$, for all Hecke eigensheaves $\E_{\calL}$ with eigenvalue $\calL$.
                \end{proof}
            ...
            \begin{theorem}[Artin Reciprocity for global function fields] \label{theorem: artin_reciprocity_for_global_function_fields}
                There exists a canonical group isomorphism $\Rep^{\cont, 1}_{\bar{\Q}_{\ell}}(\Gal(K^{\ab}/K)) \cong \scrA_{\GL_1}(X)$, sending continuous $\ell$-adic Galois characters $\chi$ to Hecke characters $\xi$, such that for all places $v$ of $K$, one has $\chi(\Frob_v) = \xi(\alpha_v)$.
            \end{theorem}
                \begin{proof}
