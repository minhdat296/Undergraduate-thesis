\section{The Automorphic Side}
    \subsection{Divisors and the Abel-Jacobi map}
        ...
        \subsubsection{Symmetric powers of curves; the Abel-Jacobi map}
            ...
            \end{definition}
            \begin{remark}[Divisors, fractional ideals, and the Abel-Jacobi map] \label{remark: abel_jacobi_map_motivation}
    ...
    \subsection{Artin Reciprocity via traces of Frobenii}
        ...
        \subsubsection{Artin Reciprocity and class field theory for global function fields over finite fields}
            ...
            \begin{theorem}[Artin Reciprocity for global function fields over finite fields] \label{theorem: artin_reciprocity_for_function_fields_over_finite_fields}
                ...
                such that $\Artin_X(\chi(\Frob_x)) = \Artin_X(\chi)([x])$ for any $\ell$-adic character $\chi \in \Rep^{\cont, 1}_{\bar{\Q}_{\ell}}(\pi_1(X_{\fet}))$ and any closed point $x \in |X|$ (here, $[x]$ denotes the line bundle corresponding to the effective divisor $[x] \in \Div_{X/\F_q}^{\eff, (1)}$; cf. proposition \ref{prop: symmetric_powers_of_curves_parametrise_divisors}).
            \end{theorem}
                \begin{proof}
                    Simply consider the following commutative diagram, wherein all the arrows are monoidal equivalences:
                        $$
                            \begin{tikzcd}
                            	{\Shv_{\underline{\bar{\Q}_{\ell}}}^{\ad, 1}(\Bun_{\GL_1}^{(1)}(X))} && {\Eig\Shv_{\underline{\bar{\Q}_{\ell}}}^1(\Bun_{\GL_1}(X))} \\
                            	{\Rep^{\cont, 1}_{\bar{\Q}_{\ell}}(\pi_1((\Bun_{\GL_1}^{(1)}(X))_{\fet}))} && {\scrA_{\GL_1}(X, \bar{\Q}_{\ell})} \\
                            	& {\Shv_{\underline{\bar{\Q}_{\ell}}}^{\ad, 1}(X)} \\
                            	& {\Rep^{\cont, 1}_{\bar{\Q}_{\ell}}(\pi_1(X_{\fet}))}
                            	\arrow["{\Autom_X}"', from=3-2, to=1-3]
                            	\arrow[from=3-2, to=4-2]
                            	\arrow["{{\text{Proposition \ref{prop: hecke_characters_from_hecke_eigensheaves}}}}", from=1-3, to=2-3]
                            	\arrow["{\Artin_X}"', dashed, from=4-2, to=2-3]
                            	\arrow["{(\AJ_{X/\F_q}^{(1)})^*}"', from=2-1, to=4-2]
                            	\arrow["{(\AJ_{X/\F_q}^{(1)})^*}"', from=1-1, to=3-2]
                            	\arrow[from=1-1, to=2-1]
                            	\arrow["{[\Autom_X]}", dashed, from=1-1, to=1-3]
                            	\arrow["{[\Artin_X]}", dashed, from=2-1, to=2-3]
                            \end{tikzcd}             
                        $$
            ...
            \begin{example}[Global class field theory for $\P^1_{\F_p}$] \label{example: global_class_field_theory_for_projective_line}
                For simplicity, suppose that $k \cong \F_p$ and that $X \cong \P^1_{\F_p}$, so that $K_X \cong \F_p(t)$; in addition, recall from example \ref{example: etale_fundamental_group_of_a_curve} that $\pi_1((\P^1_{\F_p})_{\fet}) \cong \hat{\Z}$. Now, set $K := \F_p(t)$ and note that by the Strong Approximation Theorem, we know that:
                    $$\GL_1(K)\backslash\GL_1(\A_K) \cong \prod_{v \in |\Spec \bbO_K| \setminus S_K} \bbO_v^{\x}$$
