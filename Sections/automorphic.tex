\section{The Automorphic Side}
    ...
    \subsection{Grothendieck's Sheaf-Function Correspondence and Artin Reciprocity}
        ...
        \subsubsection{Traces of Frobenii and Grothendieck's Sheaf-Function Correspondence via character sheaves}
            ...
            In order to obtain continuous $\ell$-adic characters via taking traces of Frobenii on $\ell$-adic local systems $\calF$, it is crucial that we demonstrate how for a suitable group scheme $G$, the function $\frob^{\calF}$ as in definition \ref{def: traces_of_frobenii} gives rise to a continuous group homomorphism $\xi^{\calF}: G(\F_q) \to \bar{\Q}_{\ell}^{\x}$. For this, it will be convenient to have the notion of \textbf{character sheaves} at our disposal, for which a good reference is \cite[Section 1]{cunningham_roe_function_sheaf_dictionary_quasi_characters_p_adic_tori} (whose presentation we shall also follow closely).
            \begin{definition}[Character sheaves] \label{def: character_sheaves}
                Let $k$ be a field and $G$ be a Noetherian\footnote{In \cite{cunningham_roe_function_sheaf_dictionary_quasi_characters_p_adic_tori}, the more specialised case of smooth group schemes over $\Spec \F_q$ is considered.} commutative group scheme over $\Spec k$, whose group structure is given by $\mu_G: G \x G \to G$. In such a situation, a ($\ell$-adic) \textbf{character sheaf} of $G$ shall be an $\ell$-adic local system $\E$ of rank $1$ over $G$, such that $\mu_G^*\E \cong \E \boxtimes \E$\footnote{In \cite{cunningham_roe_function_sheaf_dictionary_quasi_characters_p_adic_tori}, it is also required that character sheaves are Weil sheaves (cf. definition \ref{def: weil_sheaves}), but our omission of this extra condition will turn out to not be an issue, as Hecke eigensheaves are Weil sheaves \textit{a priori} (cf. lemma \ref{lemma: hecke_eigensheaves_are_weil_sheaves}); in addition, the Weil sheaf condition only makes sense for group schemes of prime characteristics.}.
            \end{definition}
                ...
                \begin{proof}
                    First of all, recall that for any scheme $Y$ that is locally of finite type over a field $k$, there exists a maximal $k$-subalgebra $O(Y)$ of $\Gamma(Y, \calO_Y)$ which is \'etale over $k$ (cf. \cite[Proposition 5.44]{milne_algebraic_groups}). We can then define the scheme $\pi_0(Y)$ of \say{connected components} of $Y$ to be $\Spm O(Y)$ and in doing so, we will have obtained a morphism $Y \to \pi_0(Y)$ that is universal among all \'etale $\F_q$-schemes via the fundamental adjunction:
                        $$
                            ...
                            \end{tikzcd}
                        $$
                    Now, let $G$ be a locally algebraic group over $\Spec k$, let $G_0$ be the connected component of the identity thereof. It is known that the universal morphism $G \to \pi_0(G)$ induces the following short exact sequence of locally algebraic groups (cf. \cite[Proposition 5.48]{milne_algebraic_groups}):
                        $$
                            \begin{tikzcd}
                            	1 & {G_0} & G & {\pi_0(G)} & 1
            ...
            It turns out that Hecke eigensheaves on $\Bun_{\GL_1}(X)$, which is rather fortunate for us, as this directly implies that Hecke eigensheaves correspond to certain continuous $\ell$-adic characters of $\Bun_{\GL_1}(X)(\F_q)$ via theorem \ref{theorem: sheaf_function_correspondence}.
            \begin{lemma}[Hecke eigensheaves are character sheaves] \label{lemma: hecke_eigensheaves_are_character_sheaves}
                \cite[Proposition 7.5]{tendler_2015_geometric_class_field_theory} If $\cev{H}_X: \Bun_{\GL_1}^{(1)}(X) \x \Bun_{\GL_1}(X) \to \Bun_{\GL_1}(X)$ is the group operation on $\Bun_{\GL_1}(X)$, given by $\cev{H}_X(\calL', \calL) \cong \calL' \tensor_{\calO_X} \calL$, then any Hecke eigensheaf $\E \in \Eig\Shv_{\underline{\bar{\Q}_{\ell}}}^1(\Bun_{\GL_1}(X))$ satisfies the character sheaf property with respect to $\cev{H}_X$.
            \end{lemma}
            \begin{corollary}[Traces of Frobenii define $\ell$-adic characters] \label{coro: traces_of_frobenii_define_l_adic_characters}
        ...
        \subsubsection{Artin Reciprocity for global function fields over finite fields}
            ...
            \begin{definition}[Hecke characters] \label{def: hecke_characters}
                A \textbf{Hecke character} of $X$ (or rather, of $K_X$) is a continuous $\ell$-adic character of $\GL_1(K_X)\backslash\GL_1(\A_X)/\GL_1(\bbO_{X})$. The group of Hecke characters of $X$ is denoted by $\scrA_{\GL_1}(X)$
            \end{definition}
            ...
            \begin{theorem}[Artin Reciprocity for global function fields] \label{theorem: artin_reciprocity_for_global_function_fields}
                There exists a canonical group isomorphism as below, such that for all places $v$ of $K$, one has $\chi(\Frob_v) = \xi(\alpha_v)$:
                    $$\scrA_{\GL_1}(X) \cong \Rep^1_{\bar{\Q}_{\ell}}(\Gal(K^{\ab}/K))^{\cont}$$
                    $$\xi \mapsto \chi$$
            \end{theorem}
                \begin{proof}
                    The existence of a canonical group isomorphism as stated comes directly from a combination of corollary \ref{coro: geometric_langlands_for_GL1} and theorem \ref{theorem: hecke_characters_from_hecke_eigensheaves}, so let us focus on the \say{Frobenius-compatibility} condition $\chi(\Frob_v) = \xi(\alpha_v)$.
                \end{proof}