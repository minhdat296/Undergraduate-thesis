    ...
    \subsection{Artin Reciprocity via Grothendieck's Sheaf-Function Correspondence}
        ...
        \subsubsection{Artin Reciprocity and class field theory for global function fields over finite fields}
            ...
            \end{lemma}
                ...
                \end{proof}
            \begin{theorem}[Artin Reciprocity and class field theory for global function fields over finite fields] \label{theorem: artin_reciprocity_and_class_field_theory_for_global_function_fields_over_finite_fields}
                \noindent
                \begin{enumerate}
                    \item \textbf{(Artin Reciprocity):} \cite[Theorem VI.5.5]{neukirch_2010_algebraic_number_theory} There is a group isomorphism:
                        $$\Rep^{\cont, 1}_{\bar{\Q}_{\ell}}(\pi_1(X_{\fet})) \cong \scrA_{\GL_1}(X, \bar{\Q}_{\ell})$$
                    whose inverse $\xi \mapsto \chi$ satisfies $\xi(\p) = \chi(\Frob_{\p})$ for any closed point $\p \in |X|$, viewed as a degree-$1$ effective divisor on $X$ (cf. proposition \ref{prop: symmetric_powers_of_curves_parametrise_divisors}).
                    \item \textbf{(Global class field theory):} \cite[Theorem VI.6.1]{neukirch_2010_algebraic_number_theory} 
                        \begin{enumerate}
                            \item There exists an equivalence of categories as below, such that the degree of the finite unramified abelian extensions and the indices of the subgroups coincide:
                                $$
                                    \begin{tikzcd}
                                    	{\{\text{Finite unramified abelian extensions $K/K_X$ and $K_X$-algebra homomorphisms}\}^{\op}} \\
                                    	{\{\text{Finite-index subgroups of $\GL_1(K_X)\backslash\GL_1(\A_X)/\GL_1(\bbO_{X})$}\}}
                                    	\arrow[from=1-1, to=2-1]
                                    \end{tikzcd}
                                $$ 
                            \item Furthermore, for any pair of finite unramified abelian extensions $K/K_X, K'/K_X$ corresponding to normal subgroups $N_K N_{K'} \trianglelefteq \GL_1(K_X)\backslash\GL_1(\A_X)/\GL_1(\bbO_{X})$, one has $N_{K K'} = N_K \cap N_{K'}$ and $N_{K \cap K'} = N_K N_{K'}$.
                        \end{enumerate}
                \end{enumerate}
            \end{theorem}
                \begin{proof}
                    \noindent
                    \begin{enumerate}
                        \item \textbf{(Artin Reciprocity):} The existence of a group isomorphism $\Rep^{\cont, 1}_{\bar{\Q}_{\ell}}(\pi_1(X_{\fet})) \cong \scrA_{\GL_1}(X, \bar{\Q}_{\ell})$ comes directly from combining theorem \ref{theorem: unramified_abelian_geometric_class_field_theory} with proposition \ref{prop: hecke_characters_from_hecke_eigensheaves}. 
                        
                        Before we prove the second assertion regarding the inverse of this isomorphism, let us firstly recall from corollary \ref{coro: unramified_galois_representations_induced_by_the_abel_jacobi_map} that via the Abel-Jacobi map $\AJ_{X/\F_q}^{(1)}: X \to \Bun_{\GL_1}^{(1)}(X)$, we obtain an isomorphism $\Rep^{\cont, 1}_{\bar{\Q}_{\ell}}(\pi_1(X_{\fet})) \cong \Rep^{\cont, 1}_{\bar{\Q}_{\ell}}(\pi_1((\Bun_{\GL_1}^{(1)}(X))_{\fet}))$. As such, we can instead show that the inverse map $\xi \mapsto \chi$ satisfies $\xi(\p) = \chi(\Frob_{\AJ_{X/\F_q}^{(1)}(\p)})$ for any closed point $\p \in |X|$.
                        \item \textbf{(Global class field theory):} 
                            \begin{enumerate}
                                \item First of all, we must apply lemma \ref{lemma: abelianising_continuous_characters}, which tells us that $\Rep^{\cont, 1}_{\bar{\Q}_{\ell}}(\pi_1(X_{\fet})) \cong \Rep^{\cont, 1}_{\bar{\Q}_{\ell}}(\pi_1^{\ab}(X_{\fet}))$. Then, observe that because $\bar{\Q}_{\ell}^{\x}$ (or rather, $\GL_1(\bar{\Q}_{\ell})$) is divisble by virtue of being the group of units of a field, the functor $\Comp\Ab(-, \GL_1(\bar{\Q}_{\ell})): \Comp\Ab^{\op} \to \Ab$ is exact\footnote{One can show this by applying Baer's Cirterion for Injectivity: this tells us that all divisible compact Hausdorff abelian groups $H$ are injective objects of $\Comp\Ab$, and the functors $\Comp\Ab(-, H)$ are therefore exact by the definition of injective objects.}. Consequently, we have the following commutative diagram wherein all the arrows are equivalences:
                                    $$
                                        \begin{tikzcd}[scale cd=0.9]
                                        	...
                                        	{\{\text{Finite quotients of $\Rep^{\cont, 1}_{\bar{\Q}_{\ell}}(\pi_1(X_{\fet}))$}\}} & {\{\text{Finite quotients of $\Rep^{\cont, 1}_{\bar{\Q}_{\ell}}(\pi_1(X_{\fet}))$}\}}
                                        	\arrow["{\Comp\Ab(-, \GL_1(\bar{\Q}_{\ell}))}"', from=1-1, to=2-1]
                                        	\arrow[dashed, from=1-1, to=1-2]
                                        	\arrow["{\Comp\Ab(-, \GL_1(\bar{\Q}_{\ell}))^{-1}}"', from=2-2, to=1-2]
                                        	\arrow[from=2-1, to=2-2]
                                        \end{tikzcd}
                                    $$
                                Next, recall that by remark \ref{remark: geometric_galois_correspondence}, we have an equivalence of categories (such that the degree of the class fields and the indices of the subgroups coincide):
                                    $$
                                        \begin{tikzcd}
                                        	{\{\text{Finite unramified abelian extensions $K/K_X$ and $K_X$-algebra homomorphisms}\}^{\op}} \\
                                        	{\{\text{Finite-index subgroups of $\pi_1^{\ab}(X_{\fet})$}\}}
                                        	\arrow[from=1-1, to=2-1]
                                        \end{tikzcd}
                                    $$
                                which induces an equivalence of categories as follows (again, such that the degree of the class fields and the indices of the subgroups coincide):
                                    $$
                                        \begin{tikzcd}
                                        	{\{\text{Finite unramified abelian extensions $K/K_X$ and $K_X$-algebra homomorphisms}\}^{\op}} \\
                                        	{\{\text{Finite-index subgroups of $\GL_1(K_X)\backslash\GL_1(\A_X)/\GL_1(\bbO_{X})$}\}}
                                        ...
                                        \end{tikzcd}
                                    $$
                                \item Now, note that the composite extension $K K'/K_X$ is finite, abelian, and unramified (the first two properties are trivial, and the third is due to \cite[Corollary II.7.3]{neukirch_2010_algebraic_number_theory}), and that the group $N_{K K'}$ is indeed a normal subgroup of finite index of $\GL_1(K_X)\backslash\GL_1(\A_X)/\GL_1(\bbO_{X})$, by virtue of being generated by two such subgroups, namely $N_K$ and $N_{K'}$; everything is therefore well-defined. Then, simply apply \cite[Theorem 5.13]{keith_conrad_galois_correspondence}.
                            \end{enumerate}
                    \end{enumerate}
                \end{proof}
            \begin{example}[Class field theory for $\P^1$] \label{example: class_field_theory_for_projective_line}
                For simplicity, suppose that $k \cong \F_p$ and that $X \cong \P^1_{\F_p}$ so that $K_X \cong \F_p(t)$. 
            \end{example}