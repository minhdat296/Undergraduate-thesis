\section{The Automorphic Side}
    \subsection{Symmetric powers of curves, Jacobians, and the Abel-Jacobi map}
        ...
        \begin{convention}[The setting for geometric class field theory] \label{conv: automorphic_side_conventions}
            ...
            \begin{itemize}
                ...
                \item In addition, let us now suppose for now that the base field $k$ from convention \ref{conv: base_curve} is algebraically closed. We will relax this constraint later, as this is simply a matter of convenience.
                \item Lastly, suppose that our curve $X$ from convention \ref{conv: base_curve} is, in addition, geometrically connected. 
            \end{itemize}
        \end{convention}
     ...
     \subsection{Hecke eigensheaves and geometric class field theory}
        ...
        \begin{corollary}[Geometric Langlands for $\GL_1$] \label{coro: geometric_langlands_for_GL1}
            By putting theorem \ref{theorem: galois_representations_are_sheaves_on_X} (respectively theorem \ref{theorem: tamely_ramified_abelian_geometric_class_field_theory}) and theorem \ref{theorem: unramified_abelian_geometric_class_field_theory} together, one gets a canonical monoidal equivalence of categories as follows:
    ...
    \subsection{Grothendieck's Sheaf-Function Dictionary and Hecke characters}
        ...
        We begin with the notion of Weil sheaves, which will become necessary later on (cf. theorem \ref{theorem: hecke_characters}) when we move on to computing the trace of the Frobenius endomorphism on $\ell$-adic sheaves.
        \begin{convention}[Specialising to function fields of positive characteristics] \label{conv: base_field_of_positive_characteristics}
            From now on, the base field $k$ from convention \ref{conv: base_curve} shall be some finite field $\F_q$ (which we note to be perfect).
        \end{convention}
        \begin{definition}[Weil sheaves] \label{def: weil_sheaves}
            A \textbf{Weil sheaf} on $X$ is an $\ell$-adic sheaf $\calF \in \Shv_{\underline{\bar{\Q}_{\ell}}}^{\ad, 1}(X)$ such that $\Frob_{X/k}^*\calF \cong \calF$.
        \end{definition}
        ...
        \end{proposition}
            \begin{proof}
                \cite[Proposition 5.19]{tendler_2015_geometric_class_field_theory}.
            \end{proof}
        
        Let us now move on to Grothendieck's Sheaf-Function Dictionary/Corresondence\footnote{En Français: \say{\textit{La Correspondance Faisceaux-Fonctions de Grothendieck}}.}, which allows us to formally realise the notion of Hecke eigensheaves from definition \ref{def: hecke_eigensheaves} as a categorification of the classical notion of Hecke characters (cf. theorem \ref{theorem: hecke_characters}). 
        \begin{definition}[Grothendieck's Sheaf-Function Correspondence] \label{def: traces_of_frobenii}
            For each geometric point $x \in X(k)$, we define the trace of Frobenius on an $\ell$-adic local system $\calL \in \Shv_{\underline{\bar{\Q}_{\ell}}}^{\ad, 1}(X)$ to be the trace of the Frobenius endomorphism on the stalk $\calL_x$ (which is computed in the usual manner) and write:
                $$\trace(\Frob_{X/k}^*, \calL) := \trace(\Frob_x^{\sharp}, \calL_x)$$
        \end{definition}
        \begin{proposition}[Traces of Frobenii and the the group structure on characters] \label{prop: traces_of_frobenii_and_the_group_structure_on_characters}
            For any pair of rank-$1$ $\ell$-adic local systems $\calL, \calL' \in \Shv_{\underline{\bar{\Q}_{\ell}}}^{\ad, 1}(X)$, one has:
                $$\trace(\Frob_{X/k}^*, \calL \tensor \calL') = \trace(\Frob_{X/k}^*, \calL) \trace(\Frob_{X/k}^*, \calL')$$
        \end{proposition}
            \begin{proof}
                This is a trivial consequence of the well-known fact from basic linear algebra that if $A, B$ are finite-dimensional square matrices then $\trace(A \tensor B) = \trace(A) \trace(B)$.
            \end{proof}
        \begin{convention}[Global function field of $X$] \label{conv: global_function_field}
            Let us write $K_X$ for the global function field of $X$ (i.e the stalk of the structure sheaf of $X$ at the unique generic point), $\bbO_X$ for its ring of integers, and $\A_X$ for the corresponding ring of ad\`eles. Furthermore, if $D$ is a tame Weil divisor on $X$ then we shall write $\bbO_{X \setminus D}$ for the product $\prod_{x \in \supp(D)} \widehat{\calO}_{X, x}^{\x} \x \prod_{v \not \in \supp(D)} \widehat{\calO}_{X, v}$, which will always be implicitly understood to carry the subspace topology inherited from $\bbO_X$. For more details on these constructions, we refer the reader to \cite[Section VI.1]{neukirch_2010_algebraic_number_theory}. 
        \end{convention}
        \begin{lemma}[The Lang isogeny] \label{lemma: lang_isogeny}
            If $G$ is an abelian algebraic group over $\Spec \F_q$ then there will exist a surjective topological group homomorphism $q_G: \pi_1(G_{\fet}) \to G(\F_q)$.
        \end{lemma}
            \begin{proof}
                The strategy is to make use of corollary \ref{remark: geometric_galois_correspondence}, since $G(\F_q)$ is finite and therefore corresponds uniquely to the quotient of $\pi_1(G_{\fet})$ by a normal subgroup of finite index. For that, we shall need to construct a Galois covering of $G$ with which to compute $\pi_1(G_{\fet})$; this turns out to be the \textbf{Lang isogeny} $l_G: G \to G$, which is defined by $l_G(g) := g^q - g$. For more details, see \cite[Theorem 4.4.17]{springer_linear_algebraic_groups}.
            \end{proof}
        \begin{theorem}[Hecke characters as traces of Frobenii] \label{theorem: hecke_characters}
            Let $D$ be a tame Weil divisor on $X$. Then, there is a canonical isomorphism between the group of Hecke eigensheaves of rank $1$ on $\Bun_{\GL_1}(X \setminus D)$ and that of continuous $\ell$-adic characters of $\GL_1(K_X)\backslash\GL_1(\A_X)/\GL_1(\bbO_{X \setminus D})$, so-called $\ell$-adic \textbf{Hecke characters}:
                $$\Eig\Shv_{\underline{\bar{\Q}_{\ell}}}^1(\Bun_{\GL_1}(X \setminus D)) \cong \scrA_{\GL_1}(X \setminus D)$$
        \end{theorem}
            \begin{proof}
                Suppose that we are willing to take the special case of the Weil Uniformisation Theorem for $\Bun_{\GL_1}(X)$ (cf. \cite[Proposition 3.8]{tendler_2015_geometric_class_field_theory}\footnote{For details on Weil Uniformisation for $\Bun_G(X)$, with $G$ being a general connected reductive group, see \cite{sorger_BunG_uniformisation}}) for granted, which states that the group of $k$-rational points of $\Bun_{\GL_1}(X \setminus D)$ is nothing but the ad\`elic double-quotient $\GL_1(K_X)\backslash\GL_1(\A_X)/\GL_1(\bbO_{X \setminus D})$. Next, pick a geometric point $x \in X(k)$. By making use of proposition \ref{prop: hecke_eigensheaves_are_weil_sheaves} and theorem \ref{theorem: unramified_abelian_geometric_class_field_theory} (respectively, theorem \ref{theorem: tamely_ramified_abelian_geometric_class_field_theory}) in tandem, we can consider the trace of Frobenius on the eigenvalue $\Autom_D^{-1}(\E) \in \Shv_{\underline{\bar{\Q}_{\ell}}}^{\ad, 1}(X \setminus D)$ of each Hecke eigensheaf $\E \in \Eig\Shv_{\underline{\bar{\Q}_{\ell}}}^1(\Bun_{\GL_1}(X \setminus D))$:
                    $$\rho_x(\E) := \trace(\Frob_x^{\sharp}, \Autom_D^{-1}(\E))$$
                
            \end{proof}
            
        \begin{theorem}[Artin Reciprocity for global function fields] \label{theorem: artin_reciprocity_for_global_function_fields}
            Fix a tame Weil divisor $D$ on $X$. There thus exists a canonical group isomorphism as below, such that for all places $v \not \in D$, one has $\chi(\Frob_v) = \rho(\varpi_v)$:
                $$\scrA_{\GL_1}(X \setminus D) \cong \Rep^1_{\bar{\Q}_{\ell}}(\Gal(K^{\ab}/K))^{\cont}$$
                $$\rho \mapsto \chi$$
        \end{theorem}
            \begin{proof}
                The existence of a canonical group isomorphism as stated comes directly from a combination of corollary \ref{coro: geometric_langlands_for_GL1} and theorem \ref{theorem: hecke_characters}, so let us focus on the \say{Frobenius-compatibility} condition $\chi(\Frob_v) = \rho(\varpi_v)$.
            \end{proof}